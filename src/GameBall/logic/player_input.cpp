...
PlayerInput PlayerInputController::GetInput() {
  ...
  input_.move_up = (glfwGetKey(window, GLFW_KEY_K) == GLFW_PRESS);
  input_.brake = (glfwGetKey(window, GLFW_KEY_SPACE) == GLFW_PRESS);
  input_.scale_up = (glfwGetKey(window, GLFW_KEY_EQUAL) == GLFW_PRESS);
  input_.scale_down = (glfwGetKey(window, GLFW_KEY_MINUS) == GLFW_PRESS);
